...
open class PagingMenuController: UIViewController {
    ...
    open fileprivate(set) var pagingViewControllers = [UIViewController]() {
        willSet {
            ...
            options.menuItemViewContent = newValue.compactMap({ $0.menuItemImage }).isEmpty ? .text : .image
    ...
    lazy fileprivate var isVisiblePagingViewController: (UIViewController) -> Bool = { [unowned self] in
        return self.children.contains($0)
    }
    ...
    @objc internal func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        ...
        guard let page = menuView.menuItemViews.firstIndex(of: menuItemView), page != menuView.currentPage else { return }
    ...
    fileprivate func layoutMenuView() {
        ...
        case .all:
            ...
            case .top:
                verticalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "V:|[menuView]", options: [], metrics: nil, views: viewsDictionary as [String : Any])
            case .bottom:
                verticalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "V:[menuView]|", options: [], metrics: nil, views: viewsDictionary as [String : Any])
            }
        case .menuView:
        ...
        let horizontalConstraints = NSLayoutConstraint.constraints(withVisualFormat: "H:|[menuView]|", options: [], metrics: nil, views: viewsDictionary as [String : Any])
        ...
        for (index, pagingViewController) in pagingViewControllers.enumerated() {
            ...
            if !shouldLoadPage(index) {
                ...
                if isVisiblePagingViewController(pagingViewController) {
                    ...
                    if let viewIndex = visiblePagingViewControllers.firstIndex(of: pagingViewController) {
            ...
            addChild(pagingViewController as UIViewController)
